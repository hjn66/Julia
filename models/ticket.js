          ...
          tickets.forEach(ticket => {
               ...
               if (ticket.recieveEmail && id.isValid(ticket.userId)) {
                    ...
                    User.getUserById(id, (err, user) => {
                         ...
                         Email.sendMail(user.email, 'Your ticket closed by system', mailContent, (error, info) => {
                              if (error) {
                                   Log("Method: CloseTicketAuthomaticaly, Error: " + err + " while Sending Email to " + user.email, "SYSTEM");
                              } else {
                                   Log("Method: CloseTicketAuthomaticaly, Info: Close Ticket Authomatically Email sent to " + user.email, "SYSTEM");
                              }
                         });
                    })
               }
...
module.exports.getAllTicket = function (reqUserId, reqStatus, callback) {
     var query = {};
     console.log(reqStatus + "usrId" + reqUserId);
